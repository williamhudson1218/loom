...
function openPicker(sid,el){const p=el.querySelector('.picker');if(p.dataset.open){p.dataset.open='';p.innerHTML='';return;}p.dataset.open='1';p.innerHTML='<span class="muted">finding empty panes…</span>';fetch('/api/idle-panes').then(r=>r.json()).then(panes=>{if(!panes.length){p.innerHTML='<span class="muted">No empty panes found. Open a new pane/window in tmux, then click Resume again.</span>';return;}p.innerHTML='<div class="pickhdr">Resume into which pane?</div>'+panes.map(pn=>'<button class="paneopt" data-pane="'+pn.pane_id+'">'+esc(pn.label)+(pn.cwd?'<span class="pcwd">…/'+esc(pn.cwd.split('/').filter(Boolean).pop()||'')+'</span>':'')+'</button>').join('');p.querySelectorAll('.paneopt').forEach(b=>b.onclick=()=>doResume(sid,b.dataset.pane,el,p));}).catch(()=>p.innerHTML='<span class="muted">server off?</span>');}
function doResume(sid,pane,el,p){p.innerHTML='<span class="muted">resuming…</span>';fetch('/resume?session='+encodeURIComponent(sid)+'&pane='+encodeURIComponent(pane)).then(r=>r.json()).then(j=>{flash(el,j.ok?'resumed ✓':'failed','#1f6f3f');p.dataset.open='';p.innerHTML='';}).catch(()=>{p.innerHTML='<span class="muted">server off?</span>';});}
